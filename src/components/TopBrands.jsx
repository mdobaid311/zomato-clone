...
const TopBrands = () => {
  return (
    <div className="top-brands pt-[30px] max-width">
      <div className="collection-title ">Top brands for you</div>
      <Slider {...settings}>
        {topBrandsList.map((brand) => {
          return (
            <div key={brand.id}>
              <div className="top-brands-cover h-[150px] w-[150px] rounded-[10%] bg-[color:var(--primary-bg)] shadow-lg border-[0.5px] border-solid border-[rgb(232, 232, 232)]">
                <img
                  src={brand.cover}
                  className="top-brands-image rounded-[10%] h-full w-full object-cover"
                  alt={brand.time}
                />
              </div>
            </div>
